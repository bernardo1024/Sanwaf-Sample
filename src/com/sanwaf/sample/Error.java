...
public class Error {
  ...
  public static void handle(HttpServletRequest request, HttpServletResponse response) throws IOException {
    ...
    js.append(
        "function buildHtmlTable(arr){var table=_table_.cloneNode(),columns=addAllColumnHeaders(arr,table);for(var i=0,maxi=arr.length;i<maxi;++i){var tr=_tr_.cloneNode();for(var j=0,maxj=columns.length;j<maxj;++j) {var td=_td_.cloneNode();cellValue=arr[i][columns[j]];td.appendChild(document.createTextNode(arr[i][columns[j]] || ''));tr.appendChild(td);}table.appendChild(tr);}return table;}");
    js.append(
    ...
    js.append("<script>document.getElementById('table').appendChild(buildHtmlTable(" + parmsInErrorJson
        + "));(document.getElementsByTagName('table')[0]).style=\"border-spacing: 15px; border:thick solid #FF0000\";</script>");
    sb.append(js);
